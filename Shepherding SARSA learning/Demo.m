...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
global xs ys xt yt Pt			% position variable
global xg yg Gt % goal plot properties
global S1 S2 S3 S4 S5 S6					% position strings 
global pF1 pF2 pF3							% handles for fill
global dis Down          					% handles for text display
global Cs Ct txA tx					% handles for the joint's circle
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
set(gca,'XTick',[0:50:300])	 % numbers on y-axis
set(gca,'YTick',[0:50:300])	 % numbers onf x-axis
...
xs = zeros(20,1);
ys = zeros(20,1);
xg = zeros(1,1); % goal circle - x
yg = zeros(1,1); % goal circle - y
for nsheep = 1:20
    Cs(nsheep)=plot(xs(nsheep),ys(nsheep),'ok', 'erasemode','normal');	% circle for joints
    Js(nsheep)=plot(xs(nsheep),ys(nsheep),'+k', 'erasemode','normal');  % plus for joints
end
xt = 0;
yt = 0;
Ct=plot(xt,yt, 'ok', 'erasemode','normal');		% circle at joint 'T'
Jt=plot(xt,yt, '+k', 'erasemode','normal');		% plus at joint 'T'
Gt=plot(xg,yg, 'or', 'erasemode','normal','markersize',20);		% circle at goal
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
